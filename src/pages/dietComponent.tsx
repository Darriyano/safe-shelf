...
const CardContainer: React.FC<Diet> = ({dishes}) => {
    const navigate = useNavigate();

    const action = (dishName: string, description: string, ingredients: Array<IngredientsEntity>) => {
        navigate('/details', {
        ...
        <div>
            {dishes.map((dish, index) => (
                <div key={index} className="card-display"
                     onClick={() => action(dish.name, dish.description, dish.ingredients)}>
                    <h2>{dish.name}</h2>
